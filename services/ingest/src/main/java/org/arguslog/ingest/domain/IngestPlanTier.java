 ...
 * enforce quota at the tier boundary. Wire string ({@code "free"}, {@code "pro"}, {@code
 * "enterprise"}) matches the {@code organizations.plan} enum so a single column maps to a tier in
 * either service.
 *
 * <p>TODO(P5): extract this and the api copy to a shared module — drift between the two would cause
 * "quota exceeded at api but allowed at ingest" (or vice versa) which is a hard bug to notice in
 * dashboards.
 */
