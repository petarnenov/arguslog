package dev.argus.worker.integration;
...
import dev.argus.ingest.adapter.out.auth.PostgresProjectAuthenticator;
import dev.argus.ingest.adapter.out.quota.AllowAllQuotaEnforcer;
import dev.argus.ingest.adapter.out.redis.RedisStreamEventPublisher;
import dev.argus.ingest.application.IngestEventService;
import dev.argus.ingest.application.IngestEventUseCase;
import dev.argus.ingest.application.IngestEventUseCase.Command;
import dev.argus.ingest.application.IngestEventUseCase.Result;
import dev.argus.worker.adapter.in.redis.RedisStreamEventListener;
import dev.argus.worker.adapter.in.redis.RedisStreamProperties;
import dev.argus.worker.adapter.out.fingerprint.PayloadFingerprinter;
import dev.argus.worker.adapter.out.postgres.JdbcEventStore;
import dev.argus.worker.application.ProcessEventService;
...
class IngestToPostgresEndToEndTest {
  ...
  static void boot() throws Exception {
    ...
    ProcessEventService unwrapped =
        new ProcessEventService(new PayloadFingerprinter(new ObjectMapper()), rawStore);
