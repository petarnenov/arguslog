...
/**
 * Wire format mirrors the ingest publisher's choice (string-keyed map record)
 * so the listener
 * round-trips without a custom serializer. Stream key defaults to
 * {@code events:persisted}; the
 * dispatcher consumes via the {@code worker-alerts} consumer group so
 * back-pressure cannot stall
 * event ingestion.
 */
@Component
