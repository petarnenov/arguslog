...
/**
 * Telegram bot config. {@code apiBaseUrl} is overridable so tests can point at
 * WireMock.
 *
 * @param botToken         empty in dev means "telegram dispatch is a no-op" —
 *                         we log a warn at boot instead
 *                         of crashing the worker.
 * @param dashboardBaseUrl shown in messages so an on-call can click straight to
 *                         the issue.
 */
