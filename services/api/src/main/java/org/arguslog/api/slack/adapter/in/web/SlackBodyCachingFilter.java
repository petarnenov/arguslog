...
/**
 * Buffers the request body for Slack slash-command POSTs so the controller can recompute the
 * HMAC against the exact bytes Slack signed.
 *
 ...
 * touches {@code getParameter*} — and once the input stream is drained, the controller's
 * {@code request.getReader()} returns empty. That made every signed payload mismatch and 401
 * in production even though the unit tests (which mock the verifier) were green.
 *
 * <p>Running this filter at {@link Ordered#HIGHEST_PRECEDENCE} guarantees we read the raw bytes
 * before any other filter has a chance to trigger parameter parsing. The wrapper then serves
 * the buffered bytes for every subsequent {@code getInputStream}/{@code getReader} call, so
 * both the HMAC verification AND the form-parsing in the controller see identical input.
 */
@Component
