 ...
 * resolution (Phase 2 of the per-user billing migration). Org-level callers find the org's owner
 * via membership and then ask this port — see {@link OrgPlanRepository#findPlan(long)} which
 * delegates here so existing callers keep working unchanged during the transition.
 */
public interface UserBillingRepository {
  ...
  /**
   * Active bonus snapshot for {@code userId} when an admin has comp'd them. Empty when no grant is
   * active. The plan column is the source of truth for caps; this read is dashboard metadata.
   */
