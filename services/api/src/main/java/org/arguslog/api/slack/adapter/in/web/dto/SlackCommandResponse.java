...
/**
 * Slack slash-command response envelope. {@code response_type=ephemeral} means only the user
 * who ran the command sees the reply (no channel noise); {@code in_channel} broadcasts. The
 * dispatcher picks ephemeral for read-only commands and in_channel for mutating ones (resolve,
 * ping) so the team has an audit trail.
 *
 * <p>Block Kit blocks are passed through as untyped {@code Map<String,Object>} — the structure
 * is large, well-documented at https://api.slack.com/reference/block-kit/blocks, and producing
 * a typed hierarchy would just add weight for no safety benefit.
 */
