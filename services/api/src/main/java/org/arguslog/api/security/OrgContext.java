...
/**
 * ThreadLocal holder for the org_id resolved by {@link ProjectAccessGuard} for
 * the current request.
 * Read by the persistence layer immediately before each query, so the
 * {@code SET LOCAL
 * arguslog.org_id} value lines up with the request's authorized scope. Cleared
 * in the interceptor's
 * afterCompletion to avoid leaking into the next request on a reused worker
 * thread.
 */
