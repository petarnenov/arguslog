...
/**
 * "Show me every issue introduced in this specific release" — the regression-watchlist surface
 * the dashboard renders on the release detail page. Behaviour: validate the release exists in
 * the project (so the controller surfaces 404 for unknown ids instead of a vacuous empty list),
 * then page-once through the issues table filtered by {@code first_seen_release_id}.
 *
 * <p>The release-exists check matters because an empty list otherwise looks identical to
 * "release exists but had clean shipping" vs. "release id is bogus" — distinguishing the two
 * is what gives the UI a real 404.
 */
@Service
