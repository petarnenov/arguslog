...
/**
 * Triage actions on a single issue. Status mutation (resolve/ignore/reopen) is unconditional — any
 * org member can act. Assignee mutation validates that the picked user is also a member of the same
 * org, otherwise we'd let outside accounts get attached to issues.
 */
public interface IssueTriageUseCase {
  ...
  /**
   * Auto-triage agent's hook: write a freshly-generated root-cause analysis + suggested-fix blob
   * back onto the issue. Body is opaque markdown; {@code model} is the agent's self- reported model
   * id (e.g. {@code claude-opus-4-7}). Returns empty when the issue cannot be found.
   *
   * <p>Deliberately does NOT touch status or assignee — the human still owns the triage decision.
   * The endpoint is also explicitly free of any event-emit side-effects, so a webhook alert rule
   * firing on „new error event" can't infinite-loop on the agent's own write.
   */
