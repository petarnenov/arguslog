...
/**
 * Slack OAuth install-flow config. {@code clientId}/{@code clientSecret}/{@code stateSecret} blank
 * → the install controller short-circuits to 503 with "Slack OAuth not configured" so self-hosters
 * who haven't created a Slack app yet aren't surprised by a 500.
 *
 * <p>{@code stateSecret} is intentionally separate from {@code SLACK_SIGNING_SECRET} — leaking one
 * must not let an attacker forge the other. They sit in different env vars in production.
 */
