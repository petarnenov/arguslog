...
| 4   | INFRA: Railway staging environment — services (api/ingest/worker/web) + managed Postgres+Timescale + Redis + R2 bucket + Keycloak. Secrets wiring + first deploy from `main`.                                      | ⏳ next | —         |
