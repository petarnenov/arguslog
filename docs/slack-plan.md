...
| 3   | 4     | **OAuth install flow.** `SlackOAuthService` (state JWT + token exchange) + `SlackInstallController` (`GET /api/v1/orgs/{orgId}/integrations/slack/oauth/install`, `GET /api/v1/slack/oauth/callback`). Wires `upsert()`. | ✅ done     | _next commit_ |
| 4   | 5     | **Dashboard REST API.** `IntegrationsSlackController` under `/api/v1/integrations/slack/workspaces` (`GET` list, `DELETE /{id}`, `PATCH /{id}` defaultProjectId). `SlackWorkspaceDto` excludes `install_token`. | ⏳ pending  |             |
