...
| --- | -------------------------------------------------------------------------------------------------------------------- | ------- | ------ |
| 1   | API: `/api/v1/projects/{id}/issues` paginated list — JOOQ-free JdbcTemplate, RLS-aware, real schema.                 | ⏳ next | —      |
| 2   | API auth glue — Keycloak JWT → user → org membership → `current_setting('argus.org_id')` set per request via filter. | pending | —      |
| 3   | API: issue detail + recent events endpoints; surface fingerprint, occurrence_count, level, last_seen.                | pending | —      |
