...
function frame(name: string, opts: FOpts = {}, ...children: SceneNode[]): FrameNode {
  ...
  else if (opts.fill) f.fills = [{ type: "SOLID", color: opts.fill, opacity: opts.fillOpacity ?? 1 }];
...
function button(
  ...
  const fills = {
    primary: { bg: C.primary, fg: C.textOnPrimary, border: undefined as { color: RGB; width: number } | undefined },
    secondary: { bg: C.bg, fg: C.text, border: { color: C.border, width: 1 } },
    ghost: { bg: C.surface, fg: C.text, border: undefined as { color: RGB; width: number } | undefined },
  }[variant];
  return frame(
    `btn-${variant}`,
    {
...
function statusBar(): FrameNode {
  return frame(
    ...
    frame("indicators", { dir: "H", gap: 6, cross: "CENTER" }, txt("• • •", { size: 13, color: C.text })),
  );
}
...
function buildOnboarding(): FrameNode {
  ...
  function codeLine(line: { tokens: { text: string; color?: RGB }[] }, i: number): FrameNode {
    ...
    const content = txt(
      ...
      { size: 12, mono: true, color: C.codeText, lineHeight: 18 },
    );
    let pos = 0;
...
function issueCard(row: IssueRow): FrameNode {
  ...
  const left = frame(
    "level-rail",
    { w: 4, h: "FILL", fill: lc.fg, radius: 2 },
  );
  const meta = frame(
    "meta",
...
function stackFrame(file: string, line: string, fn: string, active: boolean): FrameNode {
  return frame(
    ...
    frame(
      ...
      txt(fn + "()", { size: 13, mono: true, color: active ? C.codeFn : C.codeText, weight: "Medium" }),
...
function sparklineBars(): FrameNode {
  const heights = [12, 18, 14, 22, 30, 25, 36, 48, 40, 52, 64, 58, 46, 38, 30, 26, 22, 16, 24, 32, 40, 56, 72, 60];
  ...
  return frame(
    "bars",
    { dir: "H", gap: 4, w: "FILL", h: 80, cross: "MAX" },
    ...bars,
  );
}
...
function buildBilling(): FrameNode {
  ...
  const callout = frame(
    ...
    frame(
      ...
      txt(
        "We pause new events at the cap and notify you. We never silently charge overage.",
        { size: 12, color: hex("#047857"), lineHeight: 18, wrap: true },
      ),
    ),
  );

  function invoiceRow(opts: { date: string; plan: string; total: string; status: "paid" | "pending" }): FrameNode {
    const statusMap = {
...
function buildTokens(): FrameNode {
  function swatch(name: string, hexStr: string, color: RGB, dark = false): FrameNode {
    return frame(
      ...
      frame("chip", { w: 86, h: 56, radius: R.sm, fill: color, border: dark ? undefined : { color: C.border, width: 1 } }),
  ...
  function typeRow(label: string, sample: string, opts: TOpts): FrameNode {
    return frame(
      ...
      frame(
        ...
        txt(opts.size + "/" + (opts.lineHeight ?? opts.size! + 4) + " " + (opts.weight ?? "Regular"), {
          size: 10,
          mono: true,
          color: C.textMute,
        }),
